...
#Description: This creates the plots for the partially buried sites
...
mean_KL_temp_file = open("graph_mean_KL_intermediate_temp_data_noah.csv", "r")
mean_KL_temp_data = mean_KL_temp_file.readlines()
mean_KL_temp_file.close()
header = mean_KL_temp_data.pop(0)

mean_KL_temp_ordered_file = open("graph_mean_KL_intermediate_temp_data_ordered_noah.csv", "r")
...
for line in mean_entropy_temp_data:
    ...
    all_temp_data.append(data_array)
    #mean_KL_temp_values2 = data.pop()
    mean_entropy_temp_values_array = af.make_array(data)
    #mean_KL_temp_values2_array = make_array(mean_KL_temp_values2_array)
all_temp_mean_entropy_data_array = array(all_temp_data)


count = 0
#Make boxplot for Temp vs Mean KL for parially buried residues
...
plt.yticks([0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5], ["0.0", "0.5", "1.0", "1.5", "2.0", "2.5", "3.0", "3.5"])
plt.xticks([1, 2, 3, 4, 5, 6, 7, 8, 9], ["FB", "Soft", "0.3", "0.6", "0.9", "1.2", "1.8", "2.4", "NS"])
save_fig_title = "Mean_KL_vs_Temp_Intermediate_Boxplot_Noah" + ".pdf"
savefig(save_fig_title, format = None)
count = count + 1
#show()

#Make boxplot for Temp vs Entropy  
...
plt.xticks([1, 2, 3, 4, 5, 6, 7, 8, 9], ["FB", "Soft", "0.3", "0.6", "0.9", "1.2", "1.8", "2.4", "NS"])
save_fig_title = "Mean_Entropy_vs_Temp_Intermediate_Boxplot_Noah" + ".pdf"
savefig(save_fig_title, format = None)
count = count + 1
#show()
