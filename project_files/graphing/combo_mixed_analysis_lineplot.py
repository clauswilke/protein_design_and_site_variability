...
while (i < m): 
    ...
    p1 = ax.plot([0.1, 0.5, 0.9],cor_entropy_RSA_array, color = cm.hot_r(natural_cor_entropy_RSA), linestyle = "-", marker = "o")
    i = i + 1
...
text(-0.1, 0.8, "A", fontweight = 'bold', ha = 'center', va = 'center', fontsize = 20)
plt.xlim(0.0, 1.0)
#plt.ylim(-0.5, 0.9)
plt.yticks([-0.2, 0.0, 0.2, 0.4, 0.6, 0.8])
plt.xticks([0.1, 0.5, 0.9], ["FB, 0.1", "0.03, 0.1", "NS"])
...
for PDB in PDBS:
    RSA1, entropy_mix1 = get_noah_mixed_entropy_values(PDB, 0.0, 1.2)
...
while (i < m): 
    ...
    cor_entropy_RSA_array = correlation_values_transpose[i,:]
    natural_cor_entropy_RSA = cor_entropy_RSA_array[2]
    p1 = ax2.plot([0.1, 0.5, 0.9],cor_entropy_RSA_array, color = cm.hot_r(natural_cor_entropy_RSA), linestyle = "-", marker = "o")
    i = i + 1
...
text(-0.1, 0.8, "B", fontweight = 'bold', ha = 'center', va = 'center', fontsize = 20)
plt.xlim(0.0, 1.0)
plt.yticks([-0.2, 0.0, 0.2, 0.4, 0.6, 0.8])
plt.xticks([0.1, 0.5, 0.9], ["0.3, 1.8", "0.6, 1.8", "NS"])
plt.savefig("Combo_Lineplot_Mixed_Temp_Correlation_Plot.pdf", format = None)
#plt.show()
